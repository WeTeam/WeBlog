...
namespace Sitecore.Modules.Eviblog.Managers
{
    public class EntryManager
    {
        ...
        /// <param name="MaxNumber">The max number.</param>
        /// <returns></returns>
        public static List<Item> GetBlogEntriesAsItems(ID BlogID, int MaxNumber)
        {
            List<Item> BlogPostList = new List<Item>();
            int count = 0;

            Item Blog = Context.Database.GetItem(BlogID);

            foreach (Item entry in MakeSortedItemList(Blog.Children.ToArray()))
            {
                if (count < MaxNumber)
                {
                    BlogPostList.Add(entry);
        ...
        /// <param name="Tag">The tag.</param>
        /// <returns></returns>
        public static List<Item> GetBlogEntryItems(string Tag)
        {
            List<Item> BlogPostList = new List<Item>();
            Item Blog = Context.Database.GetItem(BlogManager.GetCurrentBlogID());
            ...
            foreach (Item entry in MakeSortedItemList(Blog.Children.ToArray()))
            {
                if (entry.Fields["Tags"].Value.Contains(Tag))
                {
                    BlogPostList.Add(entry);
                }
            }
        ...
        /// <returns></returns>
        public static List<Item> GetBlogEntryByCategorieAsItem(ID BlogID, string CategorieID)
        {
            List<Item> BlogPostList = new List<Item>();
            ...
            foreach (Item entry in MakeSortedItemList(Blog.Children.ToArray()))
            {
                if (entry.Fields["Category"].Value.Contains(CategorieID))
                {
                    BlogPostList.Add(entry);
                }
            }
        ...
        /// <param name="array">The array.</param>
        /// <returns></returns>
        public static List<Item> MakeSortedItemList(IList array)
        {
            List<Item> postItemList = new List<Item>();
            foreach (Item item in array)
            {
                if (item.TemplateID.ToString() == Settings.Default.EntryTemplateID &&
                    item.Versions.GetVersions().Length > 0)
            ...
            postItemList.Sort(new ItemDateComparerDesc());
            return postItemList;
        }
    }
}