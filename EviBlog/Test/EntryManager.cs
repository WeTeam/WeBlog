...
namespace Sitecore.Modules.WeBlog.Test
{
    ...
    public class EntryManager
    {
        ...
        /*[Test]
        public void GetAdjacentEntries_Normal()
        {
            var entries = Mod.EntryManager.GetAdjacentEntries(m_entry12);
            Assert.AreEqual(2, entries.Length);

            Assert.AreEqual(m_entry11.ID, entries[0].ID);
            Assert.AreEqual(m_entry13.ID, entries[1].ID);
        }

        [Test]
        public void GetAdjacentEntries_NoPrevious()
        {
            var entries = Mod.EntryManager.GetAdjacentEntries(m_entry11);
            Assert.AreEqual(2, entries.Length);

            Assert.IsNull(entries[0]);
            Assert.AreEqual(m_entry12.ID, entries[1].ID);
        }

        [Test]
        public void GetAdjacentEntries_NoNext()
        {
            var entries = Mod.EntryManager.GetAdjacentEntries(m_entry13);
            Assert.AreEqual(2, entries.Length);

            Assert.AreEqual(m_entry12.ID, entries[0].ID);
            Assert.IsNull(entries[1]);
        }

        [Test]
        public void GetAdjacentEntries_NonItem()
        {
            var entries = Mod.EntryManager.GetAdjacentEntries(m_category13);
            Assert.AreEqual(2, entries.Length);
        ...
        [Test]
        public void GetAdjacentEntries_Null()
        {
            var entries = Mod.EntryManager.GetAdjacentEntries(null);
            Assert.AreEqual(2, entries.Length);
